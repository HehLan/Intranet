					...
					<?php
						...
						foreach($groupes as $groupe)
						{
							if($jpt > 1)
							{
								...
								echo '<div class="row table_pool_row">';		
								echo '<table class="col-lg-12 table_pool">
									<tr>
										<th class="th_title_pool" colspan="'.($nbrteam+2).'">'.$groupe['nom_groupe'].'</th>
									</tr>
									<tr>
										<td class="td_empty_pool"></td>';
									for($i=0; $i<$nbrteam; $i++)
										echo '<th class="th_team2_pool">'.$teams[$i]['nom'].'</th>';
									echo '<th class="th_score_pool">Scores</th>
									</tr>';
									$teams2=$teams;
									$totaux='';
								...
								{
									$totaux[$team['id']]=0;
									echo '<tr class="tr_pool">
											<th class="th_team_pool">'.$team['nom'].'</th>';
											
									foreach($teams2 as $team2)
									{	
										if ($team['id']==$team2['id']) 
											echo '<td class="td_x_pool">X</td>';
										else
										{
											$couleur='same_';
											$valeur='';
											...
											echo '<td class="td_'.$couleur.'pool">'.$valeur.'</td>';
										}								
									}
									echo '<td class="td_score_pool">'.$totaux[$team['id']].'</td>	
										</tr>';
								 }
								  echo '</table>';
								  echo '</div>';
							...
							else
							{
								...
								echo '<table class="table_round">
									<tr>
										<th  class="title_round" colspan="'.($nbr_manches+2).'">'.$groupe['nom_groupe'].' 
										('.get_jour_de_la_semaine($heure).' '.get_heure($heure).')</th>
									</tr>
									<tr>
										<th class="th_empty_round"></th>';
										for($i=1;$i<=$nbr_manches;$i++)
											echo '<th class="th_manche_round">Manche '.$i.'</th>';
										echo '<th class="th_points_round">Points</th>
									</tr>';
								$inscrits='';	
								...
								foreach($totaux	as $joueur)
								{
									...
									for($i=1;$i<=$nbr_manches;$i++)
									{
										echo '<td class="td_score_round">';
										if(isset($scores[$joueur['id_joueur']][$i]))
											echo $scores[$joueur['id_joueur']][$i];
										echo '</td>';	
									}
									echo '<td class="td_total_round">'.$joueur['total'].'</td>
									</tr>';
								...
								foreach($inscrits as $inscrit)
								{
									if(!$inscrit['ok'])
									{
										...
										for($i=1;$i<=$nbr_manches;$i++) echo '<td class="td_score_round"></td>';	

										echo '<td class="td_total_round"></td>
										</tr>';
									}
								}
