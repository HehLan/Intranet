...
class EquipEsGroupEs extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	...
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `equipes_groupes` (`id_equipe`,`id_groupe`) VALUES (?,?)';
	const SQL_UPDATE='UPDATE `equipes_groupes` SET `id_equipe`=?,`id_groupe`=? WHERE `id_equipe`=? AND `id_groupe`=?';
	const SQL_SELECT_PK='SELECT * FROM `equipes_groupes` WHERE `id_equipe`=? AND `id_groupe`=?';
	const SQL_DELETE_PK='DELETE FROM `equipes_groupes` WHERE `id_equipe`=? AND `id_groupe`=?';
	const FIELD_ID_EQUIPE=-1357279369;
	const FIELD_ID_GROUPE=-1299264760;
	private static $PRIMARY_KEYS=array(self::FIELD_ID_EQUIPE,self::FIELD_ID_GROUPE);
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `equipes_groupes`'
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `equipes_groupes`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
